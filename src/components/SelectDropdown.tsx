  ...
  return (
    <div className="relative" ref={ref}>
      <button
        ...
        disabled={disabled}
        className={`flex items-center gap-1.5 px-3 py-2 bg-zinc-900 border border-zinc-800 rounded-lg text-sm transition-colors whitespace-nowrap ${
      ...
      >
        ...
        <i className={`ri-arrow-down-s-line text-zinc-500 text-base transition-transform ${open ? 'rotate-180' : ''}`}></i>
      </button>

      {open && (
        <div className="absolute top-full left-0 mt-1 z-30 bg-zinc-900 border border-zinc-800 rounded-xl shadow-2xl min-w-[100px] overflow-hidden max-h-56 overflow-y-auto">
          {options.map((opt) => (
            <button
              key={opt.value}
